...
def main():
    ...
    fail=[]; warn=[]
    report={"stage":"A00b","audit_implementation_version":"1.3-scaler-scope-fix","training_performed":False,"test_dataset_opened":False}
        ...
        for c in CITIES:
            ...
            for kind,p in (("x_scaler",xs),("y_scaler",ys)):
                if not p.exists(): fail.append(f"Missing {kind} H{H} {c}")
                    ...
                    if kind=="x_scaler" and nf!=20:
                        fail.append(f"x_scaler n_features H{H} {c}={nf}; expected 20 continuous features")
                    if kind=="y_scaler" and nf!=1:
                        fail.append(f"y_scaler n_features H{H} {c}={nf}; expected 1 target feature")
    ...
    report["x_scaler_expected_features"]=20
    report["x_scaler_scope"]="continuous_cols only; 14 cyclic/binary/no-scale predictors bypass StandardScaler"
    report["y_scaler_expected_features"]=1
