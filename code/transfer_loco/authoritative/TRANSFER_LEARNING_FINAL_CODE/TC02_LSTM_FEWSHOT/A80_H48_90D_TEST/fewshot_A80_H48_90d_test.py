...
OUT=W/"output"/"loco_fewshot"/"H48"/"test"/"90d"
...
def verify_pretest_freeze():
    ...
    if fail: raise RuntimeError("A80 pre-Test freeze failure:\n- "+"\n- ".join(fail))
    registry=pd.read_csv(reg)
    if len(registry)!=270: raise RuntimeError("A30 adapted registry row count changed")
    target=registry[(registry.horizon==48)&(registry.budget=="90d")].copy()
    if len(target)!=45: raise RuntimeError(f"A80 registry grid {len(target)} !=45")
...
def main():
    ...
    print(f"A80 H48 90d adapted Test | device={device}")
    ...
    stage={
      "status":"PASS","stage":"A80","stage_complete":True,"horizon":H,"budget":BUDGET,
    ...
    dump_json(OUT/"A80_H48_90d_stage_lock.json",stage)
    print(json.dumps(stage,indent=2))
    print("A80 H48 90d ADAPTED TEST PASS")
