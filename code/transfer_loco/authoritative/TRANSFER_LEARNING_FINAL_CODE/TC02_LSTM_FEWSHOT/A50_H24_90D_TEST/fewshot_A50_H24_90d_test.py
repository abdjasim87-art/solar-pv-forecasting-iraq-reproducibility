...
OUT=W/"output"/"loco_fewshot"/"H24"/"test"/"90d"
...
def verify_pretest_freeze():
    ...
    if fail: raise RuntimeError("A50 pre-Test freeze failure:\n- "+"\n- ".join(fail))
    ...
    if len(target)!=45: raise RuntimeError(f"A50 registry grid {len(target)} !=45")
...
def main():
    ...
    print(f"A50 H24 90d adapted Test | device={device}")
    ...
    stage={
      "status":"PASS","stage":"A50","stage_complete":True,"horizon":H,"budget":BUDGET,
    ...
    dump_json(OUT/"A50_H24_90d_stage_lock.json",stage)
    print(json.dumps(stage,indent=2))
    print("A50 H24 90d ADAPTED TEST PASS")
