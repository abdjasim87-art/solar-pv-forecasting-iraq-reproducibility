...
ROOT=W/"output"/"loco_fewshot"/"H24"/"test"/"365d"
OUT=ROOT/"raw_audit"
ZERO=W/"output"/"loco_transfer_v2"/"H24"/"cold_start"/"test"
...
SEEDS=[1,2,3]; H=24; REF=3370.0
EW=17185; EP=257775; ER=6186600; ED=3103207; EU=258120; TRUEE=171314.7648391358
...
def main():
 ...
 for seed in SEEDS:
  ...
  for city in CITIES:
   ...
   if not np.array_equal(starts,zz["window_start_idx"]) or not np.array_equal(times,zz["series_time_ns"]): fail.append(f"{city} seed{seed}: grid differs Stage60")
   if not np.array_equal(true,zz["true"]) or not np.array_equal(day,zz["daylight"]): fail.append(f"{city} seed{seed}: truth/day differs Stage60")
 ...
 report={"status":"PASS" if not fail else "FAIL","n_failures":len(fail),"failures":fail,
  ...
  "stage60_grid_truth_daylight_identity_checked":True,"raw_metrics_recomputed":True,"raw_energy_recomputed":True,"physical_bounds_checked":True}
 (OUT/"A60_raw_audit_report.json").write_text(json.dumps(report,indent=2))
