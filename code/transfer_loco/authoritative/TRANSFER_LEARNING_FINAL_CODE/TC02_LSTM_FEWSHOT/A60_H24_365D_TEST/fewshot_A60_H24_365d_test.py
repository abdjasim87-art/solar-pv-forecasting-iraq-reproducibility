...
ADAPT_ROOT=W/"output"/"loco_fewshot"/"H24"/"selection_refit"
SOURCE_ROOT=W/"output"/"loco_transfer_v2"/"H24"/"cold_start"/"selection"
ZERO_ROOT=W/"output"/"loco_transfer_v2"/"H24"/"cold_start"/"test"
OUT=W/"output"/"loco_fewshot"/"H24"/"test"/"365d"
...
EXPECTED_A40_STAGE_LOCK_SHA="05214fd1cdec3197416b93a2292d48a7fac927c692f35deb64afa8888fd27e1f"
A40_STAGE_LOCK=W/"output"/"loco_fewshot"/"H24"/"test"/"30d"/"A40_H24_30d_stage_lock.json"
EXPECTED_A50_STAGE_LOCK_SHA="848d63b43fe29f0c99c2afa79f83268870fa115b3228b188711a37188ee4ae82"
A50_STAGE_LOCK=W/"output"/"loco_fewshot"/"H24"/"test"/"90d"/"A50_H24_90d_stage_lock.json"
...
EXPECTED_WINDOWS_PER_CITY=17185
EXPECTED_WINDOWS_POOLED=257775
EXPECTED_RESIDUALS=6186600
EXPECTED_DAYLIGHT=3103207
...
def verify_pretest_freeze():
    ...
    if not A40_STAGE_LOCK.is_file():
        fail.append(f"missing closed A40 stage lock: {A40_STAGE_LOCK}")
    elif sha256(A40_STAGE_LOCK)!=EXPECTED_A40_STAGE_LOCK_SHA:
        fail.append("A40 stage-lock SHA mismatch")
    else:
        a40=load_json(A40_STAGE_LOCK)
        for k,v in [("status","PASS"),("stage_complete",True),("stage","A40"),
                    ("horizon",24),("budget","30d"),
                    ("test_dataset_opened",True),("test_inference_performed",True)]:
            if a40.get(k)!=v:
                fail.append(f"A40 prerequisite {k}={a40.get(k)!r}, expected {v!r}")
    if not A50_STAGE_LOCK.is_file():
        fail.append(f"missing closed A50 stage lock: {A50_STAGE_LOCK}")
    elif sha256(A50_STAGE_LOCK)!=EXPECTED_A50_STAGE_LOCK_SHA:
        fail.append("A50 stage-lock SHA mismatch")
    else:
        a50=load_json(A50_STAGE_LOCK)
        for k,v in [("status","PASS"),("stage_complete",True),("stage","A50"),
                    ("horizon",24),("budget","90d"),
                    ("test_dataset_opened",True),("test_inference_performed",True)]:
            if a50.get(k)!=v:
                fail.append(f"A50 prerequisite {k}={a50.get(k)!r}, expected {v!r}")
    if fail: raise RuntimeError("A60 pre-Test freeze failure:\n- "+"\n- ".join(fail))
    registry=pd.read_csv(reg)
    if len(registry)!=270: raise RuntimeError("A30 adapted registry row count changed")
    target=registry[(registry.horizon==24)&(registry.budget=="30d")].copy()
    if len(target)!=45: raise RuntimeError(f"A60 registry grid {len(target)} !=45")
...
def main():
    ...
    print(f"A60 H24 365d adapted Test | device={device}")
    ...
    for city in CITIES:
        ...
        if len(zmeta)!=len(ds) or not np.array_equal(zmeta.start_index.to_numpy(np.int64),starts):
            raise RuntimeError(f"{city}: Stage60 metadata pairing mismatch")
        if not np.array_equal(z["window_start_idx"].astype(np.int64),starts):
            raise RuntimeError(f"{city}: Stage60 start-index pairing mismatch")
        if not np.array_equal(z["series_time_ns"].astype(np.int64),times_ns):
            raise RuntimeError(f"{city}: Stage60 series-time pairing mismatch")
        ...
        for seed in SEEDS:
            ...
            if not np.array_equal(true,z["true"]): raise RuntimeError(f"{city} seed{seed}: truth differs from Stage60")
            if not np.array_equal(day,z["daylight"]): raise RuntimeError(f"{city} seed{seed}: daylight differs from Stage60")
            ...
            pairing_rows.append({"city":city,"seed":seed,"windows":len(ds),
              "starts_equal_stage60":True,"series_times_equal_stage60":True,
              "truth_equal_stage60":True,"daylight_equal_stage60":True})
    ...
    pd.DataFrame(pairing_rows).to_csv(OUT/"stage60_pairing_audit.csv",index=False)
    ...
    stage={
      "status":"PASS","stage":"A60","stage_complete":True,"horizon":H,"budget":BUDGET,
      ...
      "A40_stage_lock_sha256":sha256(A40_STAGE_LOCK),
      "A50_stage_lock_sha256":sha256(A50_STAGE_LOCK),
      "same_grid_as_stage60_zero_shot":True,"same_truth_as_stage60_zero_shot":True,
      "same_daylight_as_stage60_zero_shot":True,
    ...
    dump_json(OUT/"A60_H24_365d_stage_lock.json",stage)
    print(json.dumps(stage,indent=2))
    print("A60 H24 365d ADAPTED TEST PASS")
