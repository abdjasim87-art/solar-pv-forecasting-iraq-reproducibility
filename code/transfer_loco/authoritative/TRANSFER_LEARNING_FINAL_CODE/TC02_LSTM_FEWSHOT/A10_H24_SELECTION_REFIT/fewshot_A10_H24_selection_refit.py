...
SOURCE_STAGE = W / "output" / "loco_transfer_v2" / "H24" / "cold_start" / "selection"
OUT = W / "output" / "loco_fewshot" / "H24" / "selection_refit"
...
BUDGETS = {
    "30d": {
        ...
        "expected": {"selection_train":553,"selection_validation":121,"full_refit":697},
    },
    "90d": {
        ...
        "expected": {"selection_train":1705,"selection_validation":409,"full_refit":2137},
    },
    "365d": {
        ...
        "expected": {"selection_train":6985,"selection_validation":1729,"full_refit":8737},
    },
}
...
def bind_head_only(model) -> Tuple[List[torch.nn.Parameter], List[str], List[str]]:
    ...
    if n != EXPECTED_HEAD_PARAMS:
        raise RuntimeError(f"Expected {EXPECTED_HEAD_PARAMS} trainable H24 head parameters, got {n}")
...
def verify_prerequisites() -> Dict[str, Any]:
    ...
    if not stage_lock.is_file():
        failures.append(f"Missing frozen H24 cold-start source stage lock: {stage_lock}")
    else:
        ...
        if int(s.get("horizon",-1)) != 24 or s.get("track") != "cold_start":
            failures.append("Source stage is not H24 cold_start")
    ...
    if failures:
        raise RuntimeError("A10 prerequisite failure:\n- " + "\n- ".join(failures))
    return {
...
def execute_run(base, valid_full, target_col: str, city: str, budget: str, seed: int,
    ...
    if run_complete(run_dir):
        print(f"RESUME H24 {city} {budget} seed={seed}: complete")
    ...
    if int(fl.get("horizon",-1)) != 24 or fl.get("track") != "cold_start":
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        print(
            f"H24 {city} {budget} seed={seed} epoch={epoch:02d} "
...
# -----------------------------------------------------------------------------
# A10 runtime smoke gate — no training, no Test
# -----------------------------------------------------------------------------

def run_smoke(base, valid_full, target_col: str, prereq: Dict[str, Any]) -> Dict[str, Any]:
    smoke_dir = W / "output" / "loco_fewshot" / "H24" / "A10_smoke"
    ...
    pd.DataFrame(rows).to_csv(smoke_dir/"A10_H24_smoke_window_counts.csv", index=False)
    report = {
        ...
        "window_counts_csv_sha256":sha256(smoke_dir/"A10_H24_smoke_window_counts.csv"),
    }
    json_dump(smoke_dir/"A10_H24_smoke_report.json", report)
    print(json.dumps(report, indent=2))
    if failures:
        raise RuntimeError(f"A10 smoke failed with {len(failures)} issue(s)")
    return report
...
def main():
    ...
    base = import_file("fewshot_base_h24", BASE_CODE)
    ...
    if args.smoke_only:
        ...
        print("A10 H24 RUNTIME SMOKE PASS | no training | Test file was not read.")
        return

    # Full training is allowed only after an independently reviewed smoke PASS.
    smoke_report = W / "output" / "loco_fewshot" / "H24" / "A10_smoke" / "A10_H24_smoke_report.json"
    if not smoke_report.is_file() or safe_json(smoke_report).get("status") != "PASS":
        raise RuntimeError("Missing A10 smoke PASS. Run 00_A10_H24_smoke.sh first.")

    OUT.mkdir(parents=True, exist_ok=True)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"A10 H24 Few-shot adaptation | device={device}")
        ...
        for budget in ("30d","90d","365d"):
            for seed in SEEDS:
                try:
                    ...
                    print(f"FAIL H24 {city} {budget} seed={seed}: {exc}", file=sys.stderr)
    ...
    if len(df) != 135:
        raise RuntimeError(f"Expected 135 completed H24 adaptation runs, got {len(df)}")
    df.to_csv(OUT/"A10_H24_all_runs.csv", index=False)
    ...
    pd.DataFrame(summary_rows).to_csv(OUT/"A10_H24_selection_summary_ddof1.csv", index=False)

    # Revalidate every run lock and checkpoint before closing A10.
    manifest = []
    for city in CITIES:
        for budget in ("30d","90d","365d"):
            for seed in SEEDS:
    ...
    pd.DataFrame(manifest).to_csv(OUT/"A10_H24_checkpoint_manifest.csv", index=False)
    ...
    stage_lock = {
        ...
        "sample_sd_ddof":1,
        "prerequisites":prereq,
        "all_runs_csv_sha256":sha256(OUT/"A10_H24_all_runs.csv"),
        "selection_summary_sha256":sha256(OUT/"A10_H24_selection_summary_ddof1.csv"),
        "checkpoint_manifest_sha256":sha256(OUT/"A10_H24_checkpoint_manifest.csv"),
    }
    json_dump(OUT/"A10_H24_stage_lock.json", stage_lock)
    print(json.dumps(stage_lock, indent=2))
    print("A10 H24 FEW-SHOT SELECTION+REFIT PASS | Test file was not read.")
