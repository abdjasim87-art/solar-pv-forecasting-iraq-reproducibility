...
set -u
W=/workspace/lstm
PKG="$W/loco_fewshot_A10_H24_v1_1"
LOG="$W/loco_fewshot_A10_H24.log"
ARC="$W/loco_fewshot_A10_H24_audit.tar.gz"
...
set +e
python "$PKG/fewshot_A10_H24_selection_refit.py" 2>&1 | tee "$LOG"
STATUS=${PIPESTATUS[0]}
set -e

python "$PKG/pack_A10_audit.py" || true
echo "A10 exit status: $STATUS"
if [[ -f "$ARC" ]]; then
  sha256sum "$ARC"
fi
exit "$STATUS"
