...
OUT=W/"output"/"loco_fewshot"/"H24"/"test"/"30d"
...
def verify_pretest_freeze():
    ...
    if fail: raise RuntimeError("A40 pre-Test freeze failure:\n- "+"\n- ".join(fail))
    ...
    if len(target)!=45: raise RuntimeError(f"A40 registry grid {len(target)} !=45")
...
def main():
    ...
    print(f"A40 H24 30d adapted Test | device={device}")
    ...
    stage={
      "status":"PASS","stage":"A40","stage_complete":True,"horizon":H,"budget":BUDGET,
    ...
    dump_json(OUT/"A40_H24_30d_stage_lock.json",stage)
    print(json.dumps(stage,indent=2))
    print("A40 H24 30d ADAPTED TEST PASS")
