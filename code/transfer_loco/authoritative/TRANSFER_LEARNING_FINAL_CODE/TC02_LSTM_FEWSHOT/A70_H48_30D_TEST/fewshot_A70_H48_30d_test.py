...
OUT=W/"output"/"loco_fewshot"/"H48"/"test"/"30d"
...
def verify_pretest_freeze():
    ...
    if fail: raise RuntimeError("A70 pre-Test freeze failure:\n- "+"\n- ".join(fail))
    registry=pd.read_csv(reg)
    if len(registry)!=270: raise RuntimeError("A30 adapted registry row count changed")
    target=registry[(registry.horizon==48)&(registry.budget=="30d")].copy()
    if len(target)!=45: raise RuntimeError(f"A70 registry grid {len(target)} !=45")
...
def main():
    ...
    print(f"A70 H48 30d adapted Test | device={device}")
    ...
    stage={
      "status":"PASS","stage":"A70","stage_complete":True,"horizon":H,"budget":BUDGET,
    ...
    dump_json(OUT/"A70_H48_30d_stage_lock.json",stage)
    print(json.dumps(stage,indent=2))
    print("A70 H48 30d ADAPTED TEST PASS")
